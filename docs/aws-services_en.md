...
| **Bedrock AgentCore Runtime** | Firecracker microVM; VPC mode; idle reclaim 900s, hard cap 8h (both tunable 60–28800s) | **N** (`source_truth_agent_<projectId>`, one per project) | Reuses microVMs through a bounded Runtime session pool within each project; does not provide per-user or per-chat microVM or storage isolation |
