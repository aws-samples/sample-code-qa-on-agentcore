...
> - 正文中的「每会话独占」「用户间互不可见」属于早期设计设想。当前每项目独立 Runtime，聊天／线程映射到有界 `runtimeSessionId` 池，可借用空闲 ID 或在池满时共享 ID；每次调用新建 SDK 对话、显式回放当前追问链，并在网关进程内按 ID 串行执行。microVM 与 `/mnt/workspace` 不是用户／聊天级隔离边界；
