...
| **Bedrock AgentCore Runtime** | Firecracker microVM；VPC 模式；空闲回收 900s、硬上限 8h（均可调 60–28800s） | **N**（`source_truth_agent_<projectId>`，每项目一套） | 项目内以有界 Runtime 会话池复用 microVM；不提供用户／聊天级 microVM 或存储隔离 |
