...
策划在飞书群 @机器人 提问
  → bot-gateway（TypeScript 长驻服务，bot-gateway/src/index.ts；**每个项目一个网关进程
      ...
      · 会话路由：(chat_id / thread_id) → runtimeSessionId（src/session-map.ts，进程内 Map + TTL、有界暖池）
        —— 命中路由时复用已有 ID；新路由优先借用空闲 ID，无空闲且池满时可共享 ID
        —— 每次 invoke 新建 SDK 对话、显式回放当前追问链；网关按 runtimeSessionId 串行执行
  ...
  → AgentCore Runtime（Firecracker microVM，按 runtimeSessionId 复用；每个项目一套独立 Runtime）
      ...
      · microVM 内运行 agent-container（Python，agent-container/agent.py）
          ...
          · 临时文件使用 Session Storage /mnt/workspace（随 runtimeSessionId 复用，不是用户／聊天级存储边界）
...
| 载体 | index-service 本地副本，经 HTTP 接口服务给本项目会话 | AgentCore Session Storage `/mnt/workspace` |
| 可见性 | 本项目会话 | 使用同一 `runtimeSessionId` 的调用，可来自不同聊天／线程 |
| 生命周期 | 持久（git 仓定时 pull 刷新，分钟级反映；本地仓手动推送） | 随 Runtime 会话复用（约 14 天空闲过期） |

![会话池：每项目独立 Runtime，聊天／线程可复用池内 microVM 和 /mnt/workspace；调用共读本项目的只读 index-service 代码副本](../assets/session-isolation.svg)

机器人粒度：**每个项目一个机器人、一套独立 Runtime**。网关按 `chatId`／`threadId` 路由到有界
`runtimeSessionId` 池：新路由优先借用空闲 ID；无空闲且池未满时新建 ID，池满时共享已有 ID。
因此，聊天／线程与 microVM 并非一一对应，**microVM 和 `/mnt/workspace` 不是用户／聊天级隔离边界**。

每次调用新建 OpenAI Agents SDK 或 Claude Agent SDK 对话，由网关显式回放当前追问链，
并在本项目网关进程内按 `runtimeSessionId` 串行执行。新建 SDK 对话不等于清空复用 microVM 的文件或进程状态。
实现见 `bot-gateway/src/session-map.ts`、`followup-context.ts`、`serialize-session.ts` 及 `index.ts`，
以及 `agent-container/agent_lib.py`、`openai_runner.py`。
...
复用 `runtimeSessionId` 用于减少 microVM 冷启动，不决定 SDK 对话上下文；其关联的 microVM 和
Session Storage 可被不同聊天／线程复用，具体边界见上文「会话隔离模型」。
