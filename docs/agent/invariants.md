...
- **不变量**：会话 microVM 是 ARM64；基础镜像按 **sha256 digest** 固定、`requirements.txt` 每个直接依赖
  **`==` 精确固定**、Node 主版本固定（NodeSource `node_24.x`）。
  `@anthropic-ai/claude-code` CLI 使用 **exact semver**，镜像与 index 主机的安装版本必须一致；
  升级后须重新完成 ARM64 runtime 与术语表验证。
- **以谁为准**：`agent-container/Dockerfile`（`FROM …@sha256:…`、`node_24.x`、`claude-code@2.1.259`）、
  `agent-container/requirements.txt`（`claude-agent-sdk==0.2.103`、`httpx==0.28.1` …）、
  `index-service/requirements.txt`（`mcp==1.28.1` …）及独立术语表的 `glossary-requirements.txt` / `.lock`。
- **机检**：`scripts/check-versions.sh` —— 基础镜像必须含 `@sha256:`、每个非注释依赖必须含 `==`、
  Node 必须 `node_<N>.x`（非浮动 `node_lts.x`）、两处 `claude-code` 必须使用相同的 `@<version>`；浮动版本或版本不一致会报错。
