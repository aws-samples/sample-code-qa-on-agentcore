...
rm -f /tmp/idx.tar.gz
[ -f "$APP_STAGE/http_bridge.py" ] && [ -f "$APP_STAGE/requirements.txt" ] && [ -f "$APP_STAGE/requirements.lock" ] \
  || { rm -rf "$APP_STAGE"; echo "BOOTSTRAP_FAILED: staged index-service.tar.gz has no http_bridge.py/requirements.txt/requirements.lock — refusing to publish a broken tree over the live one"; exit 1; }
...
REQ_SIG="$(sha256sum "$APP/requirements.lock" | cut -d' ' -f1)"
PIP_STAMP=/opt/idx/.reqs_sig
_pip_needed=1
if [ "$(cat "$PIP_STAMP" 2>/dev/null || echo none)" = "$REQ_SIG" ] && python3 -m pip check >/dev/null 2>&1; then
  echo "bootstrap: python deps already match requirements.lock ($REQ_SIG) and pip check passes — skipping install (no site-packages churn under live bridges)"
  _pip_needed=0
fi
if [ "$_pip_needed" -eq 1 ]; then
  # requirements.txt is the reviewed direct-dependency intent; requirements.lock
  # is the verified ARM64/Python 3.12 transitive closure that actually ships.
  # Install only from the lock so two hosts bootstrapped at different times run
  # the same package versions. `--ignore-installed` remains necessary while the
  # host uses system Python: Ubuntu's distutils-managed packages cannot be safely
  # uninstalled in place. A dedicated /opt/idx venv remains the future migration.
  retry_net pip3 install --break-system-packages -q --ignore-installed -r "$APP/requirements.lock"
  # The exact lock prevents resolver drift; pip check catches an internally
  # inconsistent or manually edited lock before a bridge enters a restart loop.
  PIP_CHECK_OUT="$(python3 -m pip check 2>&1)" \
    || { echo "BOOTSTRAP_FAILED: locked pip dependency set is inconsistent: ${PIP_CHECK_OUT}"; exit 1; }
...
ensure_node() {
  ...
  # Configure NodeSource explicitly with its signing key. Do not execute the mutable setup_<N>.x
  # helper as root: repository configuration is data and should remain reviewable in this script.
  keyring=/etc/apt/keyrings/nodesource.asc
  if mkdir -p /etc/apt/keyrings \
    && retry_net curl -fsSL "https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key" -o "$keyring" \
    && chmod 0644 "$keyring" \
    && printf 'deb [signed-by=%s] https://deb.nodesource.com/node_%s.x nodistro main\n' "$keyring" "$NODE_MAJOR" \
      > /etc/apt/sources.list.d/nodesource.list \
    && retry_net apt-get "${APT_OPTS[@]}" update -y \
    && retry_net apt-get "${APT_OPTS[@]}" install -y nodejs; then
    return 0
  fi
...
# Keep this exact version aligned with agent-container/Dockerfile; check-versions.sh enforces both.
# Do NOT reinstall it on every re-bootstrap: `npm install -g` replaces the CLI's global
...
if [ "${GLOSSARY_ENABLED:-true}" = "false" ]; then
  ...
  elif retry_net npm install -g @anthropic-ai/claude-code@2.1.259; then
    claude --version > /opt/idx/.claude_version 2>/dev/null || true
