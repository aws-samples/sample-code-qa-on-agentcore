...
def _read_sqlite(local_path: str) -> tuple[str, bool]:
    ...
    try:
        ...
        for tbl in tables:
            ...
            # Every quote in this schema-derived identifier is doubled; LIMIT is a
            # module constant and the connection is read-only. No SQL values are interpolated.
            q = f'SELECT * FROM "{tbl.replace(chr(34), chr(34) * 2)}" LIMIT {MAX_ROWS + 2}'  # nosec B608
            # sqlite3 identifier quoting above is intentional; this is not SQLAlchemy.
...
def read_table(requested: str, *, local_root: str, repo: str = "") -> dict[str, Any]:
    ...
    # An allowed symlink name can still resolve to withheld material inside the repo.
    _withheld = served_paths.withheld_reason(
        os.path.relpath(local_path, os.path.realpath(local_root))
    )
    if _withheld:
        raise ValueError(_withheld)
