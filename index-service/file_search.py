...
def build_command(pattern: str, root: str, *, glob: str | None, max_matches: int) -> list[str]:
    ...
    to grep with extended regular expressions. Pure so it's unit-testable.
    ...
    sees hidden and ignored files too, but its regex dialect is different:
    the C-locale grep fallback uses byte-oriented POSIX ERE, not Unicode regex.
    """
    if _rg_available():
        cmd = [
        ...
        cmd.append(root)
        return cmd
    # grep fallback: -r recursive, -n line numbers, -I skip binary, -E extended regex.
    # This preserves the file visibility policy, not rg's Unicode regex semantics.
    cmd = ["grep", "-rnIE", "--exclude-dir=.git", "--exclude-dir=node_modules", "--exclude-dir=.venv"]
    if glob:
        cmd += [f"--include={glob}"]
    cmd += ["-e", pattern, root]
    return cmd
...
def run_search(
    ...
    *,
    ...
    search_env = None
    if cmd[0] == "grep":
        # In a UTF-8 locale grep -I can treat GBK source as binary. Byte-oriented
        # matching preserves ASCII/literal UTF-8 searches and still skips NUL
        # binary files. Regex "." matches one byte; CJK character ranges are
        # unsupported. Unicode regex requires rg. Decode output with replacement.
        search_env = {**os.environ, "LC_ALL": "C"}
    try:
        # build_command fixes rg/grep and passes the pattern after -e; no shell runs.
        ...
        proc = subprocess.run(  # noqa: S603 - argv list, no shell
            ...
            timeout=SEARCH_TIMEOUT_S, check=False, env=search_env,
        )
