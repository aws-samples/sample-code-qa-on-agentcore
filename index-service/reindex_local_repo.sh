...
refresh_glossary() {
  ...
  ( # subshell: a glossary hiccup must never change reindex's exit code
    ...
    # CREDENTIAL ISOLATION: same control and same reasoning as the glossary-build unit in
    # activate_project.sh (AppSec finding 9203f27c) — this is the second launch site of the
    # same root-privileged, read-tool-enabled build over an attacker-influenceable repo, so it
    # needs the same OS-level mask or the boundary only exists on one of two paths. Scoped to
    # the credential files by name rather than to /opt/idx, because /opt/idx is also the
    # application root this unit executes from (APP=/opt/idx/app). '-' prefix = ignore if the
    ...
    systemd-run --collect --unit="$glossary_unit" \
        ...
        -p "InaccessiblePaths=-/opt/idx/git-token-${PID} -/opt/idx/git-askpass-${PID}.sh -/etc/index-git-${PID}.env -/opt/idx/git-token -/etc/index-git.env" \
