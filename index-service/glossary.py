...
# Joiners cannot match identifier characters, so components have unambiguous
# boundaries; there is no nested repetition over the same character alphabet.
# nosemgrep: regex_dos
...
def is_valid_symbol(value: str) -> bool:
    # fullmatch also rejects a final newline, which a bare '$' anchor permits.
    return bool(_SYMBOL_RE.fullmatch(value or ""))


def is_valid_concept_id(value: str) -> bool:
    return bool(_CONCEPT_ID_RE.fullmatch(value or ""))
