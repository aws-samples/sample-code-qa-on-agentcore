...
# precheck decides once (only when something actually needs building); fail → SKIP + log.
  ...
  for SUBDIR in $NEED_BUILD; do
    ...
    # CREDENTIAL ISOLATION at the OS layer (AppSec finding 9203f27c). The build launches
    # Claude Code with READ TOOLS ENABLED over a checked-out copy of the customer's repo,
    # which is attacker-influenceable by anyone who can land a file in it. glossary_build.py's
    # own threat model (:74-96) enumerates three deliberate layers — CC_DISALLOWED_TOOLS,
    # --setting-sources "", CC_DENY_READ_PATHS, plus the _looks_like_credential output filter
    # — and states plainly that the residual is this unit running as ROOT, with the real fix
    # being an unprivileged user or ProtectSystem=strict pending a ReadWritePaths audit.
    # Layers 1 and 2 hold only while the CLI honours those flags: a rename or version bump
    # removes them silently. This adds the OS boundary that does not depend on the CLI's
    # cooperation — a path that is not in the unit's mount view cannot be read whatever the
    # engine decides to do.
    #
    # SCOPED TO THE CREDENTIALS, NOT TO /opt/idx. The finding recommends
    # InaccessiblePaths=/opt/idx as needing "no ReadWritePaths audit". In THIS codebase that
    # would break the build outright: /opt/idx is also the application root — APP=/opt/idx/app
    # holds glossary_worker.sh (this unit's ExecStart), and the build additionally needs
    # /opt/idx/venv, /opt/idx/bin/codegraph-server and /opt/idx/glossary-envs. So we mask the
    # actual high-value targets by name instead. Each is prefixed '-' so a host that never
    # materialized a given file (legacy host-global token, a project with no git credential)
    # still starts. /etc/index-project-*.env is deliberately NOT masked: it is passed in as
    # GLOSSARY_CONFIG_FILE below and the build must read it.
    #
    # NoNewPrivileges + dropping CAP_SYS_ADMIN close the obvious way out: systemd.exec warns that a
    # sufficiently privileged process can UNDO its own mount-namespace restrictions, and this unit
    # still runs as root, so the mask alone is only a barrier against a read tool — not against
    # native code execution. Dropping CAP_SYS_ADMIN removes the capability `mount`/`umount` need.
    # SystemCallFilter=~@mount is deliberately NOT set: it is redundant once the capability is gone
    # and a syscall filter here risks breaking a library in the build for no additional gain.
    # Running the build as a dedicated unprivileged user remains the real fix (threat model T11a).
    systemd-run --collect --unit="glossary-build-${PROJECT_ID}-${SUBDIR}" \
        -p WorkingDirectory="$APP" \
        -p MemoryMax=3G -p OOMPolicy=stop \
        -p "InaccessiblePaths=-${GIT_TOKEN_FILE} -${GIT_ASKPASS_FILE} -${GIT_ENV_FILE} -/opt/idx/git-token -/etc/index-git.env" \
