...
@pytest.fixture(params=[file_read.read_file, file_table.read_table], ids=["read_file", "read_table"])
def reader(request):
    return request.param


@pytest.mark.parametrize("target_rel", [".env", ".env.csv", ".git/config.csv", ".home/settings.csv"])
def test_reader_refuses_symlink_to_withheld_target_before_opening(
    tmp_path: Path, reader, target_rel: str, monkeypatch
) -> None:
    target = tmp_path / target_rel
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("name,value\nsynthetic_fixture,7\n", encoding="utf-8")
    (tmp_path / "visible.csv").symlink_to(target)

    def unexpected_open(*args, **kwargs):
        pytest.fail("withheld symlink target must not be opened")

    with monkeypatch.context() as patch:
        patch.setattr("builtins.open", unexpected_open)
        with pytest.raises(ValueError, match="not served"):
            reader("visible.csv", local_root=str(tmp_path))


def test_reader_keeps_withheld_symlink_name_blocked(tmp_path: Path, reader) -> None:
    target = tmp_path / "visible.csv"
    target.write_text("name,value\nsynthetic_fixture,7\n", encoding="utf-8")
    (tmp_path / ".env.csv").symlink_to(target)
    with pytest.raises(ValueError, match="not served"):
        reader(".env.csv", local_root=str(tmp_path))


@pytest.mark.parametrize("symlink_root", [False, True])
def test_reader_allows_internal_symlink(tmp_path: Path, reader, symlink_root: bool) -> None:
    # Only the target's repo-relative path matters; an ancestor may have a denied name.
    root = tmp_path / ".home" / "repo"
    (root / "data").mkdir(parents=True)
    (root / "data" / "items.csv").write_text(
        "name,value\nsynthetic_fixture,7\n", encoding="utf-8"
    )
    (root / "visible.csv").symlink_to("data/items.csv")
    local_root = root
    if symlink_root:
        local_root = tmp_path / "repo-link"
        local_root.symlink_to(root, target_is_directory=True)
    out = reader("visible.csv", local_root=str(local_root))
    assert "synthetic_fixture" in out["content"]
    assert out["path"] == "data/items.csv"


def test_reader_refuses_symlink_outside_root(tmp_path: Path, reader) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    outside = tmp_path / "outside.csv"
    outside.write_text("name,value\nsynthetic_fixture,7\n", encoding="utf-8")
    (root / "visible.csv").symlink_to(outside)
    with pytest.raises(ValueError, match="outside repo root"):
        reader("visible.csv", local_root=str(root))
