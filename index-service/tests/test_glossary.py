...
def test_identifier_validation_consumes_the_entire_value():
    # '$' alone also matches before a final newline; search seeds and IDs must
    # obey their whitelist all the way through the input.
    assert glossary.is_valid_symbol("CombatPower::Stats.calc_2")
    assert glossary.is_valid_concept_id("combat_power-2")
    for suffix in ("\n", "\r\n", "\nextra"):
        assert not glossary.is_valid_symbol("CombatPower.calc" + suffix)
        assert not glossary.is_valid_concept_id("combat_power" + suffix)
