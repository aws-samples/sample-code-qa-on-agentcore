...
def _is_loopback_host(host: str) -> bool:
    """True when `host` is a loopback literal or localhost (port already stripped).

    STRING-SHAPE CLASSIFICATION IS THE BUG THIS AVOIDS. The first version tested
    `h.startswith("127.")`, which is also true of `127.0.0.1.evil.com` — an
    attacker-controlled DNS name that resolves wherever its owner points it. That is the
    dangerous direction of false positive: the name was treated as local, so the cleartext
    warning was suppressed and CODEGRAPH_REQUIRE_TLS=1 let it through. `localhost` is now
    matched only by exact equality for the same reason (`localhost.evil.com`).

    A DNS name is deliberately NOT resolved here. A resolution result is not a security
    boundary — it can change between the check and the connection (DNS rebinding) — so a
    name that is not a numeric literal is simply not loopback for this purpose.
    """
    h = (host or "").strip().strip("[]").lower()
    if h == "localhost":
        return True
    # Dotted-decimal 127.x shorthand (`127.1`, `127.0.1`) is what the OS accepts and what a
    # health probe may legitimately carry. Anchored on both ends so a longer name that merely
    # BEGINS with 127. cannot match — then EXPANDED and validated as a real address, because the
    # shape alone would also accept `127.0.0.256`, which is not an address at all.
    #
    # AT LEAST ONE dotted group is required (`{1,3}`, not `{0,3}`). A bare `127` is not this
    # shorthand: `inet_aton` reads a SINGLE integer literal as the whole 32-bit value, so `127`
    # means 0.0.0.127 — not loopback. With `{0,3}` the expansion below turned it into 127.0.0.0
    # and reported loopback, so `http://127/mcp` skipped both the cleartext warning and the
    # CODEGRAPH_REQUIRE_TLS=1 hard failure while actually addressing a non-loopback host. That is
    # the same "shape says local, address is not" false positive this docstring says it avoids,
    # reached through the OS's own multi-format parsing instead of through DNS. A bare `127` now
    # falls through to ipaddress.ip_address("127"), which raises ValueError → False, matching the
    # single-integer meaning and erring in the non-loopback direction.
    if re.fullmatch(r"127(?:\.\d{1,3}){1,3}", h):
        parts = h.split(".")
        if len(parts) < 4:  # 127.1 → 127.0.0.1 ; 127.0.1 → 127.0.0.1
            parts = [parts[0], *["0"] * (4 - len(parts)), *parts[1:]]
        try:
            return ipaddress.ip_address(".".join(parts)).is_loopback
        except ValueError:
            return False
    try:
        ip = ipaddress.ip_address(h)
    except ValueError:
        # Not a numeric literal at all (a hostname, or a non-canonical spelling such as the
        # legacy hex form 0x7f000001, which is intentionally unsupported: normalizing every
        # historical IPv4 encoding is a bigger risk than warning about one odd spelling).
        return False
    mapped = getattr(ip, "ipv4_mapped", None)
    if mapped is not None:
        # ::ffff:127.0.0.1 is the same host as 127.0.0.1.
        return mapped.is_loopback
    return ip.is_loopback


# Hosts already warned about, so the cleartext notice is emitted once per process rather than on
# every message. Bounded in practice by the number of distinct CODEGRAPH_MCP_URL hosts a container
# is configured with — one, in every deployment shape this sample ships.
_CLEARTEXT_WARNED: set[str | None] = set()
...
def _validate_codegraph_url(url: str) -> None:
    ...
    TRANSPORT CONFIDENTIALITY (AppSec finding fc45bd79). This channel carries the
    product's core asset — file contents and search results from the customer's
    source repository — and the reasoning above only ever addressed private RANGES,
    never ENCRYPTION. It is recorded here rather than left implicit:

      * loopback → http is fine and stays unconditionally allowed. The
        `curl 127.0.0.1:<port>/health` probe is load-bearing for the runbook and ops
        (index-service/activate_project.sh:337-345), and a blanket https-only rule
        would break it — which is the kind of fix that gets reverted.
      * non-loopback + https → allowed, no comment needed.
      * non-loopback + http → ACCEPTED BY DEFAULT, but no longer silently. The
        bridge terminates no TLS today (its unit starts uvicorn with no --ssl-*
        flags), so requiring https here would break every existing deployment while
        the compensating control — VPC routing plus the index security group — is the
        one actually in force. We therefore emit a structured warning naming the
        exposure, so it appears in the runtime's own logs rather than only in a
        threat-model row, and we let a deployer make it fatal.
      * CODEGRAPH_REQUIRE_TLS=1 → non-loopback http becomes a hard failure. A
        production-intent deployment should set this once TLS is terminated on the
        bridge; it is deliberately opt-IN so that turning it on is a decision and not
        an upgrade surprise.

    A deployer copying this sample inherits the cleartext transport, so the same
    statement is carried in the deployment guide and the threat model — a reader who
    never opens this file is the one whose network placement decides whether it matters.
    """
    parsed = urlparse(url)
    ...
    if parsed.scheme == "http" and not _is_loopback_host(parsed.hostname or ""):
        require_tls = os.environ.get("CODEGRAPH_REQUIRE_TLS", "").strip().lower() in ("1", "true", "yes")
        if require_tls:
            raise ValueError(
                "CODEGRAPH_MCP_URL uses cleartext http to a non-loopback host while "
                f"CODEGRAPH_REQUIRE_TLS is set: {parsed.hostname!r}. Terminate TLS on the "
                "index bridge (uvicorn --ssl-keyfile/--ssl-certfile) and use https, or unset "
                "CODEGRAPH_REQUIRE_TLS to accept in-VPC cleartext."
            )
        # Once per process per host, not once per message. This function is reached from
        # build_options_dict → build_options → run_agent, which is the PER-MESSAGE entrypoint, so an
        # unguarded warning here writes an identical line to CloudWatch on every single invocation
        # for the whole life of a deployment whose URL is non-loopback http — which is the default
        # posture this warning describes, so it floods in exactly the case it exists to report.
        # peer_guard.log_rejection dedups for the same reason; these two are the same problem and
        # should not have been solved differently.
        if parsed.hostname not in _CLEARTEXT_WARNED:
            _CLEARTEXT_WARNED.add(parsed.hostname)
            # Best-effort: never let a logging problem break agent startup.
            try:
                logger.warning(json.dumps({
                    "event": "codegraph_cleartext_transport",
                    "host": parsed.hostname,
                    "detail": "agent→index MCP traffic carries repository source over cleartext HTTP; "
                              "confidentiality rests on VPC routing and the index security group. "
                              "Terminate TLS on the bridge and set CODEGRAPH_REQUIRE_TLS=1 before "
                              "production use. Logged once per host per process.",
                }))
...
async def run_agent(
    ...
    *,
    ...
    # Consumed below through aclosing(_drive(...)); this async generator is live code.
    ...
    try:
        ...
        for block in content:
            ...
            if tool_id is not None and name is not None:
                ...
                # Log the call START so a trace shows the actual tool sequence.
                #
                # SHAPE, NOT CONTENT (AppSec finding f2bcea2b). This used to emit a
                # 160-char preview of the tool ARGUMENTS. Those arguments are
                # model-generated FROM THE USER'S QUESTION — search terms, paths,
                # glossary lookups — and `codegraph_search_files` exists precisely to
                # search for a literal string the user named, so a pasted secret can be
                # echoed into one. This container has no redaction primitive at all
                # (unlike the gateway's redact.ts), and agent.py sets level=INFO
                # explicitly, so every call was guaranteed to reach CloudWatch.
                # :428-432 already applies the right rule one function away — it logs
                # promptChars, "never the text, which can carry a pasted secret" — and
                # this site is the same content one model hop later, so it now follows
                # the same rule. Emitting argument SIZE and the argument KEY COUNT preserves
                # everything the log line exists for ("the actual tool sequence") while
                # emitting no model-authored strings at all.
                # KEY NAMES ARE NOT EMITTED, and an earlier version of this comment was wrong
                # to claim they were safe "because they come from the MCP tool schema". Nothing
                # here enforces that: the dict is whatever the model produced, so a model that
                # invents a key echoes an arbitrary string — plausibly a fragment of the user's
                # question — straight into CloudWatch. Verified: an input keyed
                # `secret-from-user-question` was logged verbatim. A count cannot carry text.
                # NOT ported the gateway's pattern set to Python on purpose: that would
                # duplicate a control surface across two languages that must then stay in
                # sync, and finding 2b0761ff shows that pattern set already had a category
                # gap. Not emitting the value is strictly stronger than filtering it.
                # Set LOG_TOOL_ARGS=1 to opt back into value previews for local debugging;
                # it defaults closed so the exposure is a deliberate act with an owner.
                try:
                    arg = getattr(block, "input", None)
                    if os.environ.get("LOG_TOOL_ARGS", "").strip().lower() in ("1", "true", "yes"):
                        shape: dict[str, object] = {
                            "args": json.dumps(arg, ensure_ascii=False)[:160] if arg is not None else "",
                        }
                    else:
                        rendered = json.dumps(arg, ensure_ascii=False) if arg is not None else ""
                        shape = {
                            "argChars": len(rendered),
                            "argKeyCount": len(arg) if isinstance(arg, dict) else None,
                        }
                except Exception:  # noqa: BLE001
                    shape = {"argChars": 0, "argKeyCount": None}
                logger.info(json.dumps({"event": "tool_call", "tool": name, "toolUseId": str(tool_id)[:40],
                                        **shape, "traceId": trace_id}))
                continue
