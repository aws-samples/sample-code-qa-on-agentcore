...
# ── _is_loopback_host / _validate_codegraph_url (AppSec finding fc45bd79) ──
#
# The agent→index MCP channel carries repository source. Loopback may use cleartext (the
# deployed unit's /health probe depends on it); anything else gets a warning, or a hard
# failure under CODEGRAPH_REQUIRE_TLS=1. That split is only as good as the loopback test,
# and the FIRST implementation classified by string shape (`host.startswith("127.")`),
# which is also true of `127.0.0.1.evil.com` — an attacker-controlled DNS name. It was
# therefore treated as local: no warning, and TLS-required mode let it through.
#
# The dangerous direction is a NON-loopback host judged loopback, so that is what these
# cases pin. The reverse (a safe address warned about) is only a usability cost.
@pytest.mark.parametrize(
    ...
    [
        ...
        ("::1", True),
        ("[::1]", True),              # bracketed IPv6 literal
        ("127.1", True),              # dotted shorthand the OS accepts
        ("127.0.0.2", True),          # the whole 127/8 block is loopback
        ("::ffff:127.0.0.1", True),   # IPv4-mapped IPv6 is the same host
        # NOT loopback — each of these was, or could plausibly be, a bypass.
        ("127.0.0.1.evil.com", False),  # THE regression: prefix match said True
        ("localhost.evil.com", False),  # suffix confusion
        ("notlocalhost", False),        # substring confusion
        ("0.0.0.0", False),             # unspecified address is not loopback
        ("index", False),               # ordinary service DNS name
        ("", False),
        ("0x7f000001", False),          # legacy hex form: intentionally unsupported
        ("127.0.0.256", False),         # not a valid address at all
        # A BARE `127` is not the dotted shorthand, and the difference is the OS's own parsing:
        # inet_aton reads a single integer literal as the whole 32-bit value, so `127` means
        # 0.0.0.127 — NOT loopback. The first version's `{0,3}` quantifier matched it with zero
        # dotted groups and the expansion turned it into 127.0.0.0, reporting loopback, so
        # `http://127/mcp` skipped both the cleartext warning and the TLS requirement while
        # addressing a non-loopback host.
        ("127", False),
        ("0", False),                   # same class: a bare integer, and 0.0.0.0 is not loopback
    ],
)
def test_is_loopback_host_classifies_by_address_not_by_string_shape(host, expected):
    assert agent_lib._is_loopback_host(host) is expected


@pytest.mark.parametrize("host", ["127.1", "127.0.1", "127.0.0.1"])
def test_is_loopback_host_still_accepts_real_dotted_shorthand(host):
    """The fix for the bare-`127` case must not cost the shorthand a health probe may carry.

    Kept as its own test rather than only table rows: the tightening from {0,3} to {1,3} is one
    character, and this is the assertion that says which side of it must not move.
    """
    assert agent_lib._is_loopback_host(host) is True


def test_validate_codegraph_url_treats_bare_127_as_non_loopback(monkeypatch):
    """`http://127/mcp` addresses 0.0.0.127 per inet_aton, so it must NOT be exempted."""
    monkeypatch.setenv("CODEGRAPH_REQUIRE_TLS", "1")
    with pytest.raises(ValueError):
        agent_lib._validate_codegraph_url("http://127/mcp")


def test_is_loopback_host_never_resolves_dns():
    """A name that merely LOOKS local must not be probed or trusted.

    Resolution is not a security boundary — it can change between the check and the
    connection — so a non-literal host is simply not loopback here.
    """
    for host in ("localhost.attacker.example", "127.0.0.1.attacker.example"):
        assert agent_lib._is_loopback_host(host) is False


def test_validate_codegraph_url_rejects_cleartext_to_non_loopback_when_tls_required(monkeypatch):
    monkeypatch.setenv("CODEGRAPH_REQUIRE_TLS", "1")
    with pytest.raises(ValueError):
        agent_lib._validate_codegraph_url("http://index:8080/mcp")
    # The bypass must be rejected too, not just the honest hostname.
    with pytest.raises(ValueError):
        agent_lib._validate_codegraph_url("http://127.0.0.1.evil.com/mcp")


def test_validate_codegraph_url_allows_loopback_cleartext_and_any_https(monkeypatch):
    monkeypatch.setenv("CODEGRAPH_REQUIRE_TLS", "1")
    # Loopback cleartext stays legal even under the strict setting: the deployed unit's
    # own health probe uses it, and breaking that would be a self-inflicted outage.
    assert agent_lib._validate_codegraph_url("http://127.0.0.1:8080/mcp") is None
    assert agent_lib._validate_codegraph_url("https://index:8080/mcp") is None


def test_validate_codegraph_url_default_mode_allows_cleartext_but_is_not_silent(monkeypatch, caplog):
    monkeypatch.delenv("CODEGRAPH_REQUIRE_TLS", raising=False)
    # The notice is deduped per host per process, so a stale entry from another test would make
    # this assertion fail for a reason that has nothing to do with the behaviour under test.
    agent_lib._CLEARTEXT_WARNED.clear()
    with caplog.at_level("WARNING"):
        assert agent_lib._validate_codegraph_url("http://index:8080/mcp") is None
    # Cleartext is the default only because the bridge terminates no TLS; it must still be
    # visible in an incident timeline rather than an unrecorded assumption.
    assert "codegraph_cleartext_transport" in caplog.text


def test_cleartext_notice_is_emitted_once_per_host_not_once_per_message(monkeypatch, caplog):
    """The warning must not fire on every message.

    _validate_codegraph_url is reached from build_options_dict → build_options → run_agent, the
    per-message entrypoint, so an unguarded warning writes an identical CloudWatch line for every
    invocation over the life of a deployment — and it does so precisely in the default posture the
    warning describes, drowning the signal it exists to raise. peer_guard.log_rejection dedups for
    the same reason.
    """
    monkeypatch.delenv("CODEGRAPH_REQUIRE_TLS", raising=False)
    agent_lib._CLEARTEXT_WARNED.clear()
    with caplog.at_level("WARNING"):
        for _ in range(25):
            agent_lib._validate_codegraph_url("http://index:8080/mcp")
    hits = [m for m in caplog.messages if "codegraph_cleartext_transport" in m]
    assert len(hits) == 1, f"expected one notice for 25 calls, got {len(hits)}"

    # A DIFFERENT host is a different fact and must still be reported.
    with caplog.at_level("WARNING"):
        agent_lib._validate_codegraph_url("http://other-index:8080/mcp")
    hosts = {
        json.loads(m)["host"]
        for m in caplog.messages
        if "codegraph_cleartext_transport" in m
    }
    assert hosts == {"index", "other-index"}


def test_cleartext_dedup_does_not_weaken_the_tls_requirement(monkeypatch):
    """Dedup governs logging only — never the decision."""
    monkeypatch.delenv("CODEGRAPH_REQUIRE_TLS", raising=False)
    agent_lib._CLEARTEXT_WARNED.clear()
    # Warn once in default mode, so the host is now in the dedup set.
    assert agent_lib._validate_codegraph_url("http://index:8080/mcp") is None
    # Turning the requirement on must still reject that same host, silence notwithstanding.
    monkeypatch.setenv("CODEGRAPH_REQUIRE_TLS", "1")
    with pytest.raises(ValueError):
        agent_lib._validate_codegraph_url("http://index:8080/mcp")
