...
const PATTERNS: Array<[RegExp, string | ((...args: string[]) => string)]> = [
  ...
  // AWS ARNs — same internal-topology class as the two rules above, and the gap AppSec finding
  // 2b0761ff was filed on. The reachable path is NOT indexed repo content: it is a BACKEND ERROR
  // BODY. `redactSensitive` wraps the AgentCore invoke error at index.ts's invoke_http_error site,
  // and an AgentCore/Bedrock AccessDenied message names the caller's ASSUMED-ROLE ARN verbatim —
  // which discloses the runtime role NAME and the session-naming convention, i.e. what an attacker
  // would target. That is recon value, not a credential, so this is a Low: it sits with the EC2
  // private-DNS and s3:// rules, not with the token rules above.
  // Shape is unambiguous (fixed `arn:aws` prefix, 5 colon-separated fields), so unlike a bare
  // 12-digit rule this cannot collide with game data. Each field is a single negated char-class
  // terminated by a required `:` it cannot consume → no ReDoS. The
  // resource tail is a POSITIVE class of real ARN-resource characters, NOT a negated one: a negated
  // class would have to enumerate every terminator, and the first version of this rule did exactly
  // that and swallowed the CJK comma — Chinese answer text has no spaces, so an inline ARN ate the
  // rest of the sentence. That is the identical defect the s3:// rule directly above documents;
  // a positive class stops at any CJK character for free.
  // The account field is OPTIONAL (`(?:\d{12})?`): S3 and some other ARNs legitimately carry an
  // EMPTY region and account (`arn:aws:s3:::bucket/key`), and requiring exactly 12 digits there
  // silently skipped every one of them. `$` is in the resource class so a Lambda qualifier
  // (`…:function:my-fn:$LATEST`) is redacted whole instead of leaving a dangling `$LATEST`.
  [/\barn:aws[a-z0-9-]*:[^:\s]*:[^:\s]*:(?:\d{12})?:[A-Za-z0-9/_.:*+=@$-]+/gi, REDACTED],
  // Bare 12-digit AWS account id, ANCHORED to an account-id key. Deliberately NOT the unanchored
  // `\b\d{12}\b` the finding also suggested: this product exists to surface game-balance numbers to
  // planners, and a naked 12-digit rule would redact any 12-digit config value (drop weights, item
  // ids, currency caps) — the same false-positive class that made the bare-RFC1918 rule unacceptable
  // above. Anchoring accepts that a 12-digit account number sitting in prose passes through; the ARN
  // rule already covers the shape that actually appears in an AccessDenied body, which is the one
  // reachable sink. Note the key=value rule below cannot cover this: its value guard explicitly
  // SKIPS purely-numeric values, so `account_id=123456789012` falls through it by design.
  [/((?:aws[_-]?)?account[_-]?(?:id|number)["']?\s*[:=]\s*["']?)(\d{12})\b/gi,
    (_m, pre: string) => `${pre}${REDACTED}`],
];
...
export function redactSteps(steps: string[]): string[] {
  ...
  // only real narration.
  //
  // NEUTRALIZE LINKS TOO. The panel is a CardKit `markdown` element exactly like the
  // conclusion and evidence lanes, so a model-emitted `[click](https://attacker/?q=…)`
  // in a narration step renders just as clickable there — the panel was the one
  // model-output path to a markdown sink that the link-neutralization fix missed. Order
  // matches the sanitize-answer.ts contract: strip markup, neutralize renderable
  // constructs, redact LAST, so a secret split by either kind of markup is rejoined
  // BEFORE the redactor sees it.
  return steps
    .map(stripToolCallLeak)
    .map(neutralizeModelLinks)
    .map((s) => s.trim())
    .filter((s) => s.length > 0)
    .map(redactSensitive);
}
