...
// `<\/?` on purpose: a closing-only `</invoke>` (no opener, so INVOKE_BLOCK cannot pair it) was
// previously neither detected nor removed, leaving a usable splitter in the text.
const ORPHAN_INVOKE_OPEN = new RegExp(`${BOLD}<\\/?(?:antml:)?invoke\\b[^>\\n]{0,400}>${BOLD}`, "gi"); // unclosed <invoke …> or stray </invoke>
...
// DETECTION MUST COVER EVERYTHING REPLACEMENT REMOVES. This probe is deliberately derived from
// the same tag set as the replacement regexes below, because the two drifting apart is not a
// cosmetic inconsistency — it silently disables the whole function. The guard previously tested
// only OPENING `<invoke`/`<attempt_` and `function_calls`, while replacement also removed
// `<parameter>`/`</parameter>` (PARAM_BLOCK) and closing `</attempt_…>` (ORPHAN_ATTEMPT_OPEN).
// A model emitting `secret<parameter name="x"></parameter>Value` therefore fell through: the
// guard said "no markup", stripToolCallLeak returned the text untouched, the two fragments were
// each under every credential rule's minimum length, and the split secret was re-joined by the
// renderer AFTER redaction. That is exactly the leak the ordering contract in sanitize-answer.ts
// exists to prevent, defeated one layer earlier. Keep this probe and the replacement list in
// sync: any tag added below MUST be representable here.
const MARKUP_PROBE = /<\/?(?:antml:)?(?:invoke|parameter|function_calls)\b|<\/?attempt_[a-zA-Z0-9_]+\b/i;

/** True if `text` contains any (prefixed or bare) tool-call markup, opening OR closing. */
function hasToolCallMarkup(text: string): boolean {
  return MARKUP_PROBE.test(text) || BARE_TOOLCALL_RE.test(text);
}
