...
// The guard (`hasToolCallMarkup`) and the replacement list must cover the SAME tag set. When they
// drifted, stripToolCallLeak became a silent no-op for the tags only replacement knew about — and
// because sanitize-answer.ts runs redaction AFTER stripping, a secret split by such a tag was
// rejoined by the renderer with the redactor having only ever seen two short fragments.
describe("stripToolCallLeak — guard covers every tag replacement removes", () => {
  it("strips a standalone <parameter> pair and rejoins a secret split by it", () => {
    // This is the exact construction named in the AppSec finding. PARAM_BLOCK always removed
    // `<parameter>`, but the guard tested only `<invoke`/`<attempt_`/`function_calls`, so the
    // function returned early and the tag survived as a working splitter.
    const split = 'SuPerSecret<parameter name="x"></parameter>Value0123456789ABCDEFghij';
    expect(stripToolCallLeak(split)).toBe("SuPerSecretValue0123456789ABCDEFghij");
  });

  it("strips a closing-only </invoke> with no opener", () => {
    // INVOKE_BLOCK needs a pair and ORPHAN_INVOKE_OPEN matched only `<invoke`, so a lone
    // closing tag was neither detected nor removed.
    expect(stripToolCallLeak("A</invoke>B")).toBe("AB");
  });

  it("strips a closing-only </attempt_tool> with no opener", () => {
    expect(stripToolCallLeak("A</attempt_codegraph_x>B")).toBe("AB");
  });

  it("still leaves ordinary answer text untouched (guard must not over-trigger)", () => {
    for (const benign of [
      "参数说明见 Config.cs:42。",
      "调用链路是 A → B → C，见 README。",
      "负重上限 = 力量 × 1.5。",
    ]) {
      expect(stripToolCallLeak(benign)).toBe(benign);
    }
  });
});
