...
describe("order contract", () => {
  ...
  // AppSec finding 5bf12e05. The test directly above uses a CONTIGUOUS secret
  // (password=hunter2hunter2), which the keyword rule catches under EITHER ordering —
  // so it could never fail on the lane's historical redact→strip inversion, even though
  // it looks like it covers this lane. The module header claimed the order was "locked by
  // tests"; that was true for the body lane and false for this one. This test is the
  // missing oracle: a TAG-SPLIT secret, which only survives when redaction runs last.
  //
  // PRECISION NOTE (differs from the finding's write-up, verified here): the finding
  // describes the split being made by a bare `<parameter name=…>` tag and re-joined by
  // PARAM_BLOCK. A bare `<parameter>` tag on its own does NOT reach that code —
  // stripToolCallLeak's `hasToolCallMarkup` guard returns early unless the text also
  // carries `<invoke`, `function_calls`, `<attempt_…` or a bare tool-call cue, and an
  // orphan `<parameter>` matches none of them. The reachable form of the same mechanism
  // is a whole `<invoke>…</invoke>` block sitting inside the secret (the live
  // MCP-init-race leak shape, and what the body-lane test above uses): the block is
  // removed wholesale, which re-joins the halves just the same. The ordering defect and
  // its impact are exactly as filed; only the splitting token differs.
  it("live evidence: redactSensitive runs AFTER stripToolCallLeak — a secret split by a leaked <invoke> block is redacted once stripping re-joins it", () => {
    // redact.ts's BARE_TOKEN_RE needs 32 CONTIGUOUS alphanumerics. The interposed block
    // holds the halves at 11 and 25 chars, so redaction is a no-op on the split form;
    // stripToolCallLeak then removes the block and re-joins them into a 36-char
    // mixed-case-with-digits token — exactly the shape a Feishu app_secret has.
    const head = "SuPerSecret";
    const tail = "Value0123456789ABCDEFghij";
    const joined = head + tail; // 36 chars, 2+ digits, both cases → BARE_TOKEN_RE matches
    const answer = [
      "结论：见下。",
      "> 🔍 **供研发复核**",
      `> config.env:3 ${head}<invoke name="codegraph_search_files">q</invoke>${tail}`,
    ].join("\n");
    const out = sanitizeAnswerText(answer, { mode: "live" });
    expect(out.evidence).not.toContain(joined);
    expect(out.evidence).toContain(REDACTED);

    // Regression oracle: prove the INVERTED composition really does leak, so this test
    // cannot silently pass for the wrong reason (the same discipline as the body-lane
    // test above). This is the composition the lane used to run.
    const { redactSensitive } = require("../src/redact");
    const { stripToolCallLeak } = require("../src/strip-toolcall-leak");
    const inverted = stripToolCallLeak(redactSensitive(answer));
    expect(inverted).toContain(joined);
  });

  it("live evidence: clamp applies to the evidence lane too — an over-long evidence block is bounded", () => {
    // The finalize re-render clamps this element; the live lane now clamps it as well,
    // for the same reason the body is clamped (an unclamped per-frame PUT can 400,
    // and a coalesced-then-dropped write makes the card appear frozen).
    const answer = "结论：见下。\n> 🔍 **供研发复核**\n" + "> Config.cs:1 值=1\n".repeat(2000);
    const out = sanitizeAnswerText(answer, { mode: "live" });
    // The 200-char slack was unexplained and let a clamp that never ran pass as long as some
    // OTHER step happened to shorten the text. Pin what the clamp actually guarantees, deriving
    // the bound from clampForCard ITSELF so a change to the truncation wording cannot make this
    // test either stale or falsely green.
    const bound = clampForCard("x".repeat(MAX_CARD_EVIDENCE_CHARS * 3), MAX_CARD_EVIDENCE_CHARS);
    expect(out.evidence).toContain("已截断");   // positive evidence the clamp executed
    expect(out.evidence.length).toBeLessThanOrEqual(bound.length);
    // And it must genuinely have clamped, not merely be short by accident.
    expect(out.evidence.length).toBeGreaterThan(MAX_CARD_EVIDENCE_CHARS / 2);
  });
