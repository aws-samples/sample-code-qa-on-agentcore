...
describe("redactSensitive", () => {
  ...
  // AppSec finding 2b0761ff. The reachable sink is a BACKEND ERROR BODY: redactSensitive
  // wraps the AgentCore invoke error, and an AccessDenied message names the caller's
  // assumed-role ARN — disclosing the role name and session-naming convention.
  it("redacts an assumed-role ARN in a realistic AccessDenied body", () => {
    const arn = "arn:aws:sts::123456789012:assumed-role/source-truth-runtime-role/st-abc123";
    const body = `AccessDeniedException: User: ${arn} is not authorized to perform: bedrock:InvokeModel`;
    const out = redactSensitive(body);
    expect(out).not.toContain("arn:aws");
    expect(out).not.toContain("source-truth-runtime-role");
    expect(out).not.toContain("123456789012");
    expect(out).toContain("[已隐藏]");
    // The surrounding diagnostic prose must survive — this log line exists for triage.
    expect(out).toContain("is not authorized to perform");
  });

  it("redacts every ARN partition/service shape, and stops at prose boundaries", () => {
    for (const arn of [
      "arn:aws:iam::123456789012:role/idx-svc",
      "arn:aws-cn:s3:cn-north-1:123456789012:accesspoint/ap",
      "arn:aws:bedrock:us-east-1:123456789012:inference-profile/p-1",
    ]) {
      const out = redactSensitive(`前缀 ${arn}，后续说明。`);
      expect(out).not.toContain("123456789012");
      // Tight termination: the CJK comma after the ARN must not be swallowed, or an
      // inline citation would eat the rest of the sentence (no spaces in Chinese).
      expect(out).toContain("，后续说明。");
    }
  });

  // Both cases below were found by adversarial review of the first version of the ARN rule, which
  // required a literal 12-digit account field and omitted `$` from the resource class. Each shipped
  // as a silent MISS — the rule reported success while leaking.
  it("redacts ARNs whose region and account fields are legitimately EMPTY", () => {
    // S3 (and several other services) omit region+account entirely. The first version required
    // exactly 12 digits there, so it skipped every ARN of this extremely common shape.
    const out = redactSensitive("（arn:aws:s3:::my-bucket/path/key.json）之后重试");
    expect(out).not.toContain("my-bucket");
    expect(out).toContain("[已隐藏]");
    // Full-width parens must survive: the positive resource class is what makes this work.
    expect(out).toContain("）之后重试");
  });

  it("redacts a Lambda ARN together with its $LATEST qualifier, leaving no fragment", () => {
    const out = redactSensitive("arn:aws:lambda:us-east-1:123456789012:function:my-fn:$LATEST");
    expect(out).not.toContain("123456789012");
    expect(out).not.toContain("my-fn");
    // The first version left a dangling "$LATEST" because `$` was absent from the class. A
    // fragment is not just untidy: it confirms to a reader that a Lambda ARN was present.
    expect(out).not.toContain("$LATEST");
    expect(out).toBe("[已隐藏]");
  });

  it("redacts a keyed 12-digit account id but NOT a bare 12-digit game value", () => {
    const keyed = redactSensitive("account_id=123456789012 是部署账号");
    expect(keyed).not.toContain("123456789012");
    expect(keyed).toContain("[已隐藏]");
    // Anchored on purpose: this product surfaces balance numbers to planners, so an
    // unanchored \d{12} rule would redact ordinary config values. A naked 12-digit
    // number in prose passes through, and the ARN rule covers the shape that actually
    // reaches a log.
    const bare = redactSensitive("掉落权重上限是 123456789012 点");
    expect(bare).toContain("123456789012");
  });

  it("does not redact this gateway's own traceId or a CardKit card id", () => {
    // Regression guard for the two identifiers operators grep by — the new numeric
    // rule must not have widened into them.
    const out = redactSensitive("card 7382910473 trace st-96f2a7f42c25b1049e3d8a7c6f150b23");
    expect(out).toContain("7382910473");
    expect(out).toContain("st-96f2a7f42c25b1049e3d8a7c6f150b23");
  });
...
describe("redactSteps", () => {
  // The reasoning panel is a CardKit `markdown` element exactly like the conclusion and the
  // evidence lane, but it was the one model-output path to a markdown sink that the
  // link-neutralization work missed — redactSteps did strip → redact with no neutralize step,
  // so a model-emitted link in a narration step stayed clickable for the whole group.
  it("neutralizes model-emitted links in a narration step (panel is a markdown sink)", () => {
    const out = redactSteps(["Inspect [click](https://attacker.example/?q=snippet)"]);
    expect(out).toEqual(["Inspect click（https://attacker.example/?q=snippet）"]);
    expect(out[0]).not.toContain("](");
  });

  it("removes a model-emitted image marker in a narration step", () => {
    const out = redactSteps(["![x](https://attacker.example/pixel.png)"]);
    expect(out[0]).not.toMatch(/!\[/);
    expect(out[0]).toContain("图片已移除");
  });

  it("rejoins and redacts a secret split by tool-call markup inside a step", () => {
    // Order contract applies here too: strip markup, neutralize, redact LAST.
    const out = redactSteps(['SuPerSecret<parameter name="x"></parameter>Value0123456789ABCDEFghij']);
    expect(out.join("")).not.toContain("SuPerSecretValue");
  });
