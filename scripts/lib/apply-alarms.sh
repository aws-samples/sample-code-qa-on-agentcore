...
# `${arr[@]+"${arr[@]}"}`, NOT `"${arr[@]}"`. Under `set -u` (line 23), bash BEFORE 4.4 treats
# expanding an EMPTY array as an unbound variable and aborts. AL2 ships bash 4.2; AL2023 ships
# 5.2, where it expands to nothing. So the plain form works on every development machine here
# and dies on AL2 — which is exactly what happened: this line aborted the script, and because
# --dry-run leaves TOPIC_ARN empty by design it aborted on the dry-run path every time, failing
# all three dry-run assertions in test_alarms.sh and blocking merge on thirteen consecutive
# dry-run builds with no error text (see that file's check() for why the text was missing).
# Do not "simplify" this back: no test on a bash 4.4+ machine can catch the regression.
PLAN="$(python3 "$RENDER" "$THRESHOLDS" --namespace "$NAMESPACE" --prefix "$PREFIX" ${TOPIC_ARG[@]+"${TOPIC_ARG[@]}"})" || {
...
' <<< "$line")
  ...
  if aws cloudwatch put-metric-alarm --region "$REGION" \
      ...
      ${actions_arg[@]+"${actions_arg[@]}"} >/dev/null 2>&1; then
