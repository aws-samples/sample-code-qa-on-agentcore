...
VPCE_SG=""
VPCE_IDS=""
VPCE_ECR_IDS=""   # subset of VPCE_IDS: the ECR interface endpoints, which get the scoped policy
if [[ "${DEPLOY_VPC_ENDPOINTS:-true}" == "true" ]]; then
  ...
  # ENDPOINT POLICIES (AppSec finding 6ee01ebb). Created without --policy-document, all three
  # endpoints inherit the AWS default endpoint policy, which is literally
  #   {"Effect":"Allow","Principal":"*","Action":"*","Resource":"*"}
  # so any principal that can route to them may reach ANY ECR registry and ANY S3 bucket its IAM
  # permits — including buckets in accounts unrelated to this deployment. That is the EGRESS leg
  # that composes with the two other index-host findings (the root-privileged glossary build, and
  # the unauthenticated bridge): the index host holds a checked-out copy of the customer's source
  # and a read-only git credential, so an unrestricted S3 path is what turns a read into an
  # exfiltration.
  #
  # NOTE the SG is a DIFFERENT control and does not cover this — easy to conflate. The
  # source-truth-vpce group above governs who can REACH the endpoint; the endpoint policy governs
  # what the endpoint will DO for them. Both are needed; only the first was present.
  #
  # Resolved here (not at the sts call further down) because the endpoints are created above it.
  #
  # RETRIED, and only attempted when the policy block below will actually consume it. A single
  # unguarded attempt made a transient STS throttle indistinguishable from "there is no account id",
  # and the consumer refuses to continue without one — so without the retry an API blip would fail
  # an otherwise fine deploy. Under DEPLOY_VPCE_POLICIES=false the id is not needed here at all and
  # its absence is not an error.
  if [[ -z "${ACCOUNT:-}" && "${DEPLOY_VPCE_POLICIES:-true}" == "true" ]]; then
    for _acct_try in 1 2 3; do
      ACCOUNT="$(aws sts get-caller-identity --query Account --output text 2>/dev/null || echo "")"
      [[ -n "$ACCOUNT" && "$ACCOUNT" != "None" ]] && break
      say warn "account id lookup attempt ${_acct_try}/3 failed — retrying"
      sleep $(( _acct_try * 3 ))
    done
  fi
  ACCOUNT="${ACCOUNT:-}"
  ...
  ensure_endpoint() { # <short-name> <service-suffix> <Interface|Gateway>
    ...
    # Track the ECR endpoints separately: only they receive the aws:PrincipalAccount policy. The
    # S3 gateway endpoint must keep the AWS default or ECR layer downloads break — see the
    # ENDPOINT POLICIES block below the ensure_endpoint calls.
    case "$1" in ecr-*) VPCE_ECR_IDS="$VPCE_ECR_IDS $id" ;; esac
    printf '%s' "$id"
  }
  ...
  VPCE_ECR_IDS="$(printf '%s' "$VPCE_ECR_IDS" | tr -s ' ' | sed 's/^ //; s/ $//')"

  # Apply the scoped endpoint policy (AppSec finding 6ee01ebb — see the block above the
  # ensure_endpoint definition for why this control is needed and why the SG does not cover it).
  #
  # Applied AFTER create rather than via create-vpc-endpoint --policy-document, deliberately:
  # validate_iam_policies.py can only audit a document it can find and parse STATICALLY, and its
  # parser anchors on a trailing redirect. A `--policy-document` inside the `$( … --output text)`
  # command substitution above has no redirect to anchor on, so it would be counted as present and
  # never validated — the exact silent-skip the validator's own comment documents (a review once
  # slipped a full account-escalation chain past it that way). One post-create `modify` call keeps
  # the document inline, auditable, and identical on the create and the re-run path. The cost is a
  # brief window after creation where the default policy applies; a just-created endpoint is
  # `pending` and carries no traffic, so nothing routes through it in that window.
  #
  # ECR ENDPOINTS ONLY — the S3 gateway endpoint is deliberately left on the AWS default.
  # An earlier version applied this same aws:PrincipalAccount condition to all three endpoints,
  # which BREAKS IMAGE PULLS: an ECR pull fetches the manifest from ECR and then downloads the
  # layers from `prod-<region>-starport-layer-bucket` over the S3 endpoint, and that request is
  # not authenticated as a principal in the deploying account, so the condition denies it — as a
  # container-start failure rather than a legible AccessDenied. AWS's own documented minimum S3
  # endpoint policy for ECR keeps `Principal: "*"` and narrows Action/Resource instead, precisely
  # because principal-based conditions do not fit this path
  # (https://docs.aws.amazon.com/AmazonECR/latest/userguide/vpc-endpoints.html#ecr-minimum-s3-perms).
  # Narrowing S3 to `s3:GetObject` on the starport bucket is the correct next step, but it cannot
  # be verified without a real pull from this environment and this deploy also stages artifacts in
  # S3, so it is left to the deployer. The residual gap is recorded in docs/security-threat-model.md
  # (T11b) rather than papered over. DEPLOY_VPCE_POLICIES=false skips this block entirely.
  if [[ "${DEPLOY_VPCE_POLICIES:-true}" == "true" && -n "$ACCOUNT" && "$ACCOUNT" != "None" ]]; then
    for _vpce in $VPCE_ECR_IDS; do
      [[ -n "$_vpce" && "$_vpce" != "None" ]] || continue
      # Skip only when the live document already carries the condition key AND this account's id.
      # Checking the key name alone (the first version) treated a policy conditioned on the WRONG
      # account — or one carrying the key in an unrelated statement — as converged, and then never
      # touched it again while reporting success. Still not a full semantic comparison of
      # Action/Resource; a deployer who hand-edits those should re-check them.
      _live="$(Q describe-vpc-endpoints --vpc-endpoint-ids "$_vpce" \
        --query 'VpcEndpoints[0].PolicyDocument' --output text 2>/dev/null || echo "")"
      if [[ "$_live" == *"aws:PrincipalAccount"* && "$_live" == *"$ACCOUNT"* ]]; then
        continue
      fi
      # FAIL CLOSED. The first version logged a warning and let the deploy exit 0, so a throttle or
      # a transient API error left the endpoint fully open while the run reported success — the
      # half-applied state a security control must never leave silently. Retry the transient cases,
      # then stop the deploy. Opting out is explicit (DEPLOY_VPCE_POLICIES=false), not the accident
      # of an API failure.
      _pol_ok=false
      for _try in 1 2 3; do
        if Q modify-vpc-endpoint --vpc-endpoint-id "$_vpce" --policy-document "{\"Version\":\"2012-10-17\",\"Statement\":[{\"Effect\":\"Allow\",\"Principal\":\"*\",\"Action\":\"*\",\"Resource\":\"*\",\"Condition\":{\"StringEquals\":{\"aws:PrincipalAccount\":\"${ACCOUNT}\"}}}]}" >/dev/null 2>&1; then
          _pol_ok=true
          break
        fi
        say warn "endpoint $_vpce policy update attempt ${_try}/3 failed — retrying"
        sleep $(( _try * 3 ))
      done
      if [[ "$_pol_ok" == "true" ]]; then
        say ok "endpoint $_vpce policy scoped to account ${ACCOUNT}"
      else
        say err "could not scope the policy on $_vpce after 3 attempts — it would keep the AWS default (Principal:*, Action:*, Resource:*). Refusing to continue with an open endpoint; re-run, or set DEPLOY_VPCE_POLICIES=false to accept the default deliberately."
        exit 1
      fi
    done
  elif [[ "${DEPLOY_VPCE_POLICIES:-true}" == "true" ]]; then
    # FAIL CLOSED, for the same reason as the modify path above and with the same blast radius.
    # By this point the endpoints EXIST and are carrying the AWS default (Principal:*, Action:*,
    # Resource:*). Warning here and continuing left them that way while the run reported success —
    # the half-applied, silently-open state finding 6ee01ebb was filed to remove, reached through a
    # transient STS error instead of a transient EC2 one. The lookup is retried above; if the id
    # still cannot be resolved, accepting the open default has to be a decision, not an API blip.
    say err "could not resolve the account id after 3 attempts — the ECR endpoints would keep the AWS default (Principal:*, Action:*, Resource:*). Refusing to continue with open endpoints; re-run, or set DEPLOY_VPCE_POLICIES=false to accept the default deliberately."
    exit 1
  fi
