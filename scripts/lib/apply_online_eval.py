        ...
        try:
            ...
            if (role_just_created and attempt < ROLE_RETRY_ATTEMPTS
                ...
                print(f"  · 执行角色尚未传播，{ROLE_RETRY_SLEEP_S} s 后重试 "
                      ...
                      file=sys.stderr)
                # Bounded IAM propagation backoff: at most five waits, only for a new role.
...
def config_name(runtime_name: str, runtime_id: str) -> str:
    ...
    # Non-security name suffix, not authentication/integrity. Preserve existing
    # resource names so a rerun cannot create duplicate evaluation configurations.
    # nosemgrep: insecure-hash-algorithm-sha1
    digest = hashlib.sha1(runtime_id.encode("utf-8"), usedforsecurity=False).hexdigest()[:6]
