...
run_stage() { # run_stage <name> : true if <name> was selected (or no --only given)
  [[ ${#ONLY[@]} -eq 0 ]] && return 0
  local s; for s in ${ONLY[@]+"${ONLY[@]}"}; do [[ "$s" == "$1" ]] && return 0; done
  return 1
}
...
if run_stage dashboards; then
  ...
  bash "$LIB/apply-dashboards.sh" ${COMMON[@]+"${COMMON[@]}"} ${EXTRA[@]+${EXTRA[@]+"${EXTRA[@]-}"}} || warn_fail apply-dashboards dashboards
fi
if run_stage filters; then
  say step "monitoring: metric-filters"
  if [[ ${#EXTRA[@]} -gt 0 ]]; then
    # custom flags (e.g. --defs/--list) → a single forwarded invocation
    bash "$LIB/apply-metric-filters.sh" ${COMMON[@]+"${COMMON[@]}"} ${EXTRA[@]+${EXTRA[@]+"${EXTRA[@]-}"}} || warn_fail apply-metric-filters filters
  else
    bash "$LIB/apply-metric-filters.sh" ${COMMON[@]+"${COMMON[@]}"} \
    ...
    bash "$LIB/apply-metric-filters.sh" ${COMMON[@]+"${COMMON[@]}"} --defs "$BYPROJ_DEFS" \
      || warn_fail "apply-metric-filters (by-project)" filters
  fi
fi
if run_stage alarms; then
  say step "monitoring: alarms"
  bash "$LIB/apply-alarms.sh" ${COMMON[@]+"${COMMON[@]}"} ${EXTRA[@]+${EXTRA[@]+"${EXTRA[@]-}"}} || warn_fail apply-alarms alarms
fi
if run_stage dau; then
  say step "monitoring: DAU lambda"
  bash "$LIB/apply-dau-lambda.sh" ${COMMON[@]+"${COMMON[@]}"} ${EXTRA[@]+${EXTRA[@]+"${EXTRA[@]-}"}} || warn_fail apply-dau-lambda dau
fi
...
if run_stage observability; then
  ...
  bash "$LIB/apply-observability.sh" "$REGION" ${OBS_FLAGS[@]+"${OBS_FLAGS[@]}"} || warn_fail apply-observability observability
fi
