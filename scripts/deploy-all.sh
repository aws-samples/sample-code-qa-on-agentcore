...
# 0.20.1 was protocol-verified against the then-pinned mcp==1.23.3 client (MCP 2024-11-05; the three
...
if skip artifacts; then say warn "skip artifacts"; elif [[ "$DRY_RUN" == true ]]; then
  ...
  cp "$ROOT"/index-service/*.py "$ROOT"/index-service/*.sh \
    "$ROOT"/index-service/requirements.txt "$ROOT"/index-service/requirements.lock "$IDX_STAGE"/
  ...
  ( cd "$IDX_STAGE" && det_tar ./*.py ./*.sh requirements.txt requirements.lock glossary-requirements.lock | gzip -n > "$TMP_IDX" )
  rm -rf "$IDX_STAGE"
...
probe_failed_summary() {  # one line per rc-3 project; printed before BOTH the exit-1 and exit-3 paths
  ...
  local _p
  for _p in ${_PROBE_FAILED[@]+${_PROBE_FAILED[@]+"${_PROBE_FAILED[@]-}"}}; do
