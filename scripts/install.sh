...
flow_init_env() {
  ...
  exec "$SCRIPT_DIR/deploy-all.sh" --region "$REGION" ${HW_FLAGS[@]+"${HW_FLAGS[@]}"} \
    ${EXTRA_FLAGS[@]+"${EXTRA_FLAGS[@]}"} ${PROBE_FLAG[@]+"${PROBE_FLAG[@]}"} --skip-projects ${LOCAL_FLAG[@]+"${LOCAL_FLAG[@]}"} \
    ${DOMAIN_FLAG[@]+"${DOMAIN_FLAG[@]}"} ${LOCALE_FLAG[@]+"${LOCALE_FLAG[@]}"}
}
...
json.dump(cfg,open(sys.argv[1],"w"),ensure_ascii=False,indent=2)' "$PROJECTS_CFG"
  ...
  "$SCRIPT_DIR/deploy-all.sh" --region "$REGION" --skip-projects ${LOCAL_FLAG[@]+"${LOCAL_FLAG[@]}"} \
    ${EXTRA_FLAGS[@]+"${EXTRA_FLAGS[@]}"} ${PROBE_FLAG[@]+"${PROBE_FLAG[@]}"} ${DOMAIN_FLAG[@]+"${DOMAIN_FLAG[@]}"} ${LOCALE_FLAG[@]+"${LOCALE_FLAG[@]}"} \
...
flow_redeploy() {
  ...
  local SEL; choose_project SEL ${PIDS[@]+"${PIDS[@]}"}
  ...
  if [[ "$rebootstrap" == true ]]; then
    ...
    "$SCRIPT_DIR/deploy-all.sh" --region "$REGION" --skip-projects ${LOCAL_FLAG[@]+"${LOCAL_FLAG[@]}"} \
      ${GLOS_FLAG[@]+"${GLOS_FLAG[@]}"} ${GMF_FLAG[@]+"${GMF_FLAG[@]}"} ${MON_FLAG[@]+"${MON_FLAG[@]}"} ${PROBE_FLAG[@]+"${PROBE_FLAG[@]}"} ${DOMAIN_FLAG[@]+"${DOMAIN_FLAG[@]}"} ${LOCALE_FLAG[@]+"${LOCALE_FLAG[@]}"} \
