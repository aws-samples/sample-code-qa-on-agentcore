...
check() {
  ...
  printf '  FAIL %s\n' "$1"
  _fail=$((_fail+1))
  # Print the captured output, when the caller passed it as $3.
  #
  # An assertion that CAPTURES a command's stdout+stderr and then checks only its exit status
  # discards the only description of the failure. These dry-run assertions did that, and the
  # result was a build failure reproduced THIRTEEN times across CR revisions with no error text
  # in any log — every investigation had to work backwards from which assertions failed. The
  # capture already exists; not printing it was the whole defect. Any assertion whose subject is
  # a captured run should pass that capture here.
  if [[ -n "${3:-}" ]]; then
...
# $dry carries stdout+stderr; it is passed to check() as the third argument so a failure here
# reports WHY rather than only which assertion tripped. All three share one invocation, so all
# three report the same capture — that is deliberate: whichever one a reader looks at first
# carries the diagnosis.
dry="$("$APPLY" --dry-run --region us-east-1 2>&1)"; rc=$?
check "apply --dry-run exits 0" "$rc" "$dry"
[[ "$dry" == *"ToolcallLeakDetected"* && "$dry" == *"LogPipelineStalled"* ]]
check "dry-run lists alarms" $? "$dry"
[[ "$dry" == *"NOT created"* ]]
check "dry-run notes no SNS/actions" $? "$dry"
