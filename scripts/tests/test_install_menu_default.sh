...
_run=0 _fail=0
# check <name> <rc> [captured-output] : on failure, print the captured output if one was passed.
# The third argument is why this exists. Every rc assertion below already captured install.sh's
# combined output into $out and then reported nothing but the number, so a failing build said
# "expected 0, got 1" and not one word about what install.sh had said — the same defect revision 14
# fixed in test_alarms.sh, in this same file, unnoticed because the capture was already there.
check() {
  _run=$((_run+1))
...
# python3 is deliberately NOT stubbed out. install.sh's dependency check requires it, and twenty-odd
# later paths really parse JSON with it, so an `exit 0` stub would make every one of those a lie.
# What the sandbox needs is the REAL interpreter reachable from its stripped PATH — and that PATH
# used to be `$T/bin:/usr/bin:/bin`, i.e. it assumed /usr/bin/python3. True on a dev machine, FALSE
# on the AL2 build image, where python3 comes from the build's own interpreter directory. `env -i`
# dropped it, install.sh's dependency check exited 1 before any flow ran, and 28 assertions failed
# at once with rc 1 — while the argument-parsing ones (they exit BEFORE that check) kept passing,
# which is what made it look like a behavioural regression in install.sh rather than an absent
# interpreter. Reproduced both ways: replacing `python3` in that check with an absent name fails the
# same 28 assertions here AND on a pristine mainline archive, so the gap predates this branch.
# A forwarding shim rather than another PATH entry, because the interpreter may itself be a wrapper
# script whose directory is not self-contained; exec'ing its absolute path needs no PATH at all.
PY3="$(command -v python3 2>/dev/null || true)"
if [[ -z "$PY3" ]]; then
  # A versioned name is not a fallback for tidiness: some managed interpreter directories ship
  # python3.10 / python3.12 and NO bare `python3`, so requiring that one name would make this suite
  # unrunnable on the very image it needs to run on. The shim below supplies the name install.sh
  # checks for; which interpreter it forwards to is a separate question, answered here.
  for _c in python3.13 python3.12 python3.11 python3.10 python3.9 python3.8; do
    PY3="$(command -v "$_c" 2>/dev/null || true)"
    [[ -n "$PY3" ]] && break
  done
fi
if [[ -z "$PY3" ]]; then
  # Fail, do not skip. Every assertion below executes install.sh, so with no interpreter there is
  # nothing this suite can verify and a silent skip would be the "reports green having tested
  # nothing" outcome. Name the missing dependency so the next reader does not have to re-derive it
  # from a wall of rc-1 failures.
  printf '  FAIL no python3 interpreter found — install.sh requires one; this suite cannot run\n'
  printf '  1 run, 1 failed\n'
  exit 1
fi
printf '#!/bin/sh\nexec %s "$@"\n' "$PY3" > "$T/bin/python3"
...
# One unconditional info line, not an assertion. Three consecutive dry runs failed inside this
# sandbox with nothing in the log describing the sandbox itself, so each round cost a build to learn
# one fact about it. This prints the fact up front.
printf '  info sandbox interpreter: %s\n' "$PY3"

run_install() {  # run_install <args...>  (prints combined output; rc in $?)
  # PATH is `$T/bin` FIRST — the stubs and the python3 shim must win — then the caller's own PATH.
  # It used to be `$T/bin:/usr/bin:/bin`, which is not isolation but an assumption: that everything
  # install.sh shells out to exists in a minimal /usr/bin. On the AL2 build image it does not, and
  # the failure surfaces as an exit code with no message, which is unfalsifiable from a test log.
  # `env -i` is still what isolates: it clears the ENVIRONMENT, so no AWS_*, no inherited region, no
  # host deploy-config leaks in. Tool availability was never what it was protecting, and pinning
  # PATH to two directories only made the suite depend on which image it happened to run on.
  env -i PATH="$T/bin:$PATH" HOME="$T" NO_COLOR=1 bash "$T/scripts/install.sh" "$@" </dev/null 2>&1
}
...
# These two assert WHICH call is made and WHICH flags ride along — deliberately not the exact
# expansion syntax. They used to match `"${LOCAL_FLAG[@]}"` verbatim, which made them a second
# copy of install.sh's formatting: changing every possibly-empty array to the bash-4.2-safe
# `${arr[@]+"${arr[@]}"}` form broke them while the behaviour they describe was untouched. That
# is the same maintain-it-twice shape this change set keeps hitting, so the assertions now name
# the array and let either expansion form satisfy them.
[[ "$_rd" == *'"$SCRIPT_DIR/deploy-all.sh" --region "$REGION" --skip-projects'* && "$_rd" == *'LOCAL_FLAG[@]'* ]]; check "redeploy runs deploy-all --skip-projects (the only path that re-bootstraps the host)" $?
_flags_ok=1
# _first_index <haystack> <needle> : how many characters precede <needle>, or -1 when it is absent.
# BOTH ordering assertions below go through it. Writing the prefix-strip twice by hand is what
# produced `${#_rd%%GLOS_FLAG*}` here: bash refuses that outright (`${#var}` cannot be combined with
# `%%pattern`), and the refusal aborts the whole `[[ … ]]` BEFORE its `|| _flags_ok=0` can run — so
# _flags_ok stayed 1, the assertion PASSED, and the only trace was one "bad substitution" line on
# stderr. An assertion that cannot fail is worse than no assertion, and it hid itself as well.
_first_index() {  # _first_index <haystack> <needle>
  local _pre="${1%%"$2"*}"
  [[ "$_pre" == "$1" ]] && { printf '%s\n' -1; return 0; }
  printf '%s\n' "${#_pre}"
}
# Scope the forwarding + ordering checks to THAT invocation's own (continued) lines, not to the
# whole function body. In the body GLOS_FLAG first appears at `flag_value "${GLOS_FLAG[0]:-}"` and
# MON_FLAG a few lines below it, so "GLOS_FLAG precedes MON_FLAG in flow_redeploy" holds whatever
# order the arguments are actually forwarded in — the first version of this assertion compared
# exactly those two positions and so could not fail. Caught by swapping the forwarded order in
# install.sh and watching it still pass, which is the second defect found in this one assertion.
_call="$(printf '%s\n' "$_rd" | sed -n '/"\$SCRIPT_DIR\/deploy-all.sh"/,/[^\\]$/p')"
[[ -n "$_call" ]]; check "redeploy: the deploy-all invocation is extractable (the checks below are void without it)" $?
for _v in GLOS_FLAG GMF_FLAG MON_FLAG; do [[ "$_call" == *"$_v[@]"* ]] || _flags_ok=0; done
# order matters: glossary, then its cap, then monitoring
_i_glos="$(_first_index "$_call" 'GLOS_FLAG')"
_i_gmf="$(_first_index "$_call" 'GMF_FLAG')"
_i_mon="$(_first_index "$_call" 'MON_FLAG')"
[[ "$_i_glos" -ge 0 && "$_i_gmf" -ge 0 && "$_i_mon" -ge 0 \
   && "$_i_glos" -lt "$_i_gmf" && "$_i_gmf" -lt "$_i_mon" ]] || _flags_ok=0
[[ "$_flags_ok" -eq 1 ]]; check "redeploy forwards the opt-in flags to that deploy-all call, in order" $?
# deploy-all must run BEFORE deploy_project.sh in the function body — these two needles each occur
# once in it, so this pair stays on $_rd deliberately.
_i_all="$(_first_index "$_rd" '"$SCRIPT_DIR/deploy-all.sh"')"
_i_proj="$(_first_index "$_rd" 'bash "$SCRIPT_DIR/lib/deploy_project.sh"')"
[[ "$_i_all" -ge 0 && "$_i_proj" -ge 0 && "$_i_all" -lt "$_i_proj" ]]
check "redeploy: deploy-all (re-bootstrap) precedes deploy_project.sh" $?
...
[[ $rc -eq 0 ]]; check "--yes --action redeploy, one project → exit 0 (got $rc)" $? "$out"
...
reset_host true demo
out="$(run_install --yes --action=redeploy --region us-east-1)"; rc=$?
[[ $rc -eq 0 && -s "$T/dp.log" ]]; check "--action=redeploy (equals form) works too (rc $rc)" $? "$out"
...
[[ $rc -eq 0 && ! -s "$T/da.log" && "$(persisted DEPLOY_GLOSSARY_MAX_FILES)" == 400 ]]; check "fast path: unchanged flags → no deploy-all, cap persisted by install.sh (rc $rc)" $? "$out"
...
[[ $rc -eq 0 ]]; check "F2: 3 projects, glossary flipped → exit 0 (got $rc)" $? "$out"
...
[[ $rc -eq 0 && "$(cat "$T/dp.log")" == "us-east-1 alpha NO_PROBE=0" ]]; check "F2: no glossary change → only the selected project is redeployed" $? "$out"
...
check "failed project remains pending after the global choice was committed" $? "$out"
rm -f "$T/dp_fail"; : > "$T/dp.log"
out="$(run_install --yes --action redeploy --project alpha --region us-east-1)"; rc=$?
[[ $rc -eq 0 && "$(cat "$T/dp.log")" == $'us-east-1 alpha NO_PROBE=0\nus-east-1 beta NO_PROBE=1' ]]
check "retry without glossary flags reconciles the unfinished project" $? "$out"
[[ -z "$(persisted DEPLOY_GLOSSARY_PENDING_PROJECTS)" ]]
check "successful retry clears the pending project list" $? "$out"
...
check "deleted pending beta is skipped while the remaining pending gamma is redeployed" $? "$out"
[[ -z "$(persisted DEPLOY_GLOSSARY_PENDING_PROJECTS)" ]]
check "deleted projects are removed from the durable pending list" $? "$out"
: > "$T/dp.log"
out="$(run_install --yes --action redeploy --project alpha --region us-east-1)"; rc=$?
[[ $rc -eq 0 && "$(cat "$T/dp.log")" == "us-east-1 alpha NO_PROBE=0" ]]
check "a second redeploy no longer retries or fails on the deleted project" $? "$out"
